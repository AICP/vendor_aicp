...
KERNEL_MAKE_FLAGS += -j$(shell prebuilts/tools-aicp/$(HOST_PREBUILT_TAG)/bin/nproc --all)

TOOLS_PATH_OVERRIDE := \
    LD_LIBRARY_PATH=$(BUILD_TOP)/prebuilts/tools-aicp/$(HOST_PREBUILT_TAG)/lib:$$LD_LIBRARY_PATH \
    PERL5LIB=$(BUILD_TOP)/prebuilts/tools-aicp/common/perl-base
...
ifeq (,$(filter 5.10, $(TARGET_KERNEL_VERSION)))
    ...
    ifeq ($(KERNEL_ARCH),arm64)
        ...
        TOOLS_PATH_OVERRIDE += PATH=$(BUILD_TOP)/prebuilts/tools-aicp/$(HOST_PREBUILT_TAG)/bin:$(KERNEL_TOOLCHAIN_arm):$$PATH
    else
        TOOLS_PATH_OVERRIDE += PATH=$(BUILD_TOP)/prebuilts/tools-aicp/$(HOST_PREBUILT_TAG)/bin:$$PATH
    endif
    ...
    TOOLS_PATH_OVERRIDE += PATH=$(BUILD_TOP)/prebuilts/tools-aicp/$(HOST_PREBUILT_TAG)/bin:$(CLANG_PREBUILTS)/bin:$$PATH
endif
