...
function lineageremote()
{
    ...
    local AICP_USER=$(git config --get review.review.lineageos.org.username)
    if [ -z "$AICP_USER" ]
    then
        git remote add lineage ssh://review.lineageos.org:29418/$PFX$PROJECT
    else
        git remote add lineage ssh://$AICP_USER@review.lineageos.org:29418/$PFX$PROJECT
    fi
...
function githubremote()
{
    ...
    git remote add github https://github.com/AicpOS/$PROJECT
...
function lineagerebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "AicpOS Gerrit Rebase Usage: "
