...
if not depsonly:
    print("Device %s not found. Attempting to retrieve device repository from LineageOS Github (http://github.coAICP)." % device)

repositories = []

if not depsonly:
    githubreq = urllib.request.Request("https://raw.githubusercontent.coAICP/mirror/main/default.xml")
    try:
...
def get_default_or_fallback_revision(repo_name):
    ...
    try:
        stdout = subprocess.run(
            ["git", "ls-remote", "-h", "https://:@github.coAICP/" + repo_name],
