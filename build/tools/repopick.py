...
if __name__ == '__main__':
    ...
    for item in mergables:
        ...
        # Try fetching from our Gerrit if using default gerrit
        if args.gerrit == default_gerrit:
            if args.verbose:
                print('Trying to fetch the change from our Gerrit')

            if args.pull:
                cmd = ['git pull --no-edit', default_gerrit + '/' + item['project'], item['fetch'][method]['ref']]
            else:
                cmd = ['git fetch', default_gerrit + '/' + item['project'], item['fetch'][method]['ref']]
            if not args.verbose:
                cmd.append('--quiet')
            if args.verbose:
                print(cmd)
        ...
        # Check if it worked
        if args.gerrit != default_gerrit:
            # If not using the default gerrit.
            if args.verbose:
